...
def test_trigger_consumer_invalid_queue():
    ...
    data = {"queue": 123}
    with pytest.raises(ValueError, match="The 'queue' field must be a non-empty string if provided."):
...
def test_trigger_consumer_invalid_max_retries():
    ...
    with pytest.raises(ValueError, match="The 'max_retries' field must be a non-negative integer if provided."):
        TriggerConsumer.from_dict(data)
...
def test_trigger_consumer_invalid_retry_delay():
    ...
    with pytest.raises(ValueError, match="The 'retry_delay' field must be a non-negative integer if provided."):
        TriggerConsumer.from_dict(data)
...
def test_trigger_consumer_invalid_batch_size():
    ...
    with pytest.raises(ValueError, match="The 'batch_size' field must be a non-negative integer if provided."):
        TriggerConsumer.from_dict(data)
...
def test_trigger_consumer_invalid_dead_letter_queue():
    ...
    with pytest.raises(ValueError, match="The 'dead_letter_queue' field must be a non-empty string if provided."):
        TriggerConsumer.from_dict(data)
